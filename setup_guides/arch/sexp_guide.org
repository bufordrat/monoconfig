#+TITLE: arch instructions for sexp
...
** create iwd config file for connecting to 802.1x network
+ =/var/lib/iwd/eduroam.8021x=
  #+begin_src ini
    [Security]
    EAP-Method=PEAP
    EAP-Identity=anonymous@uchicago.edu
    EAP-PEAP-Phase2-Method=MSCHAPV2
    EAP-PEAP-Phase2-Identity=your-cnetid@uchicago.edu
    EAP-PEAP-Phase2-Password=your-cnetid-password
  #+end_src
** restart =iwd=
#+begin_example
...
#+end_example
+ change =wlan0= to your device name, where applicable
...
#+begin_example
  ...
  # fdisk /dev/nvme0n1
#+end_example
+ choose EFI partition for =/boot= partition and ext4 for everything
  else
+ recommended size for =/boot=: 1 GB
+ no =/home= partition on =sexp=; shove the remainder in =/= partition
+ =/boot= is unencrypted
...
** pacstrap
#+begin_example
  # pacstrap -K /mnt base base-devel linux linux-firmware dhcpcd lvm2 vim efivar emacs git openssh
#+end_example
** generate fstab
#+begin_example
...
+ base it on =sequent='s bootloader files
+ consult =generate_boot_loader= rule in makefile for reference
+ I don't recommend running that rule
+ just get the UUID out of =/etc/fstab= and put it in the options line
...
#+begin_example
  # useradd -m -u 1158 -g 11158 -G wheel -s /usr/bin/zsh teichman
#+end_example
+ uid: 1158
+ gid: 11158
+ groups: wheel
...
** don't forget to install =dhcpcd= and =systemctl enable= it before rebooting
