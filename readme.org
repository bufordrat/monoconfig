...
In its idiomatic use, =monoconfig= encourages the following
(informally-enforced) lifestyle policies:
...
+ everything else on your system, including the contents of =~= that
  are not in =~/data=, is to be treated disposable, because it can be
  reinstalled
